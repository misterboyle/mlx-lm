...
- Use `--parent <id>` when creating tasks under an epic to establish hierarchy
...
Use a Python 3.8+ venv in the repo root:
...
- TurboQuant server flags:
  ...
  - `--turbo-v-bits N` — use standard affine quantization for values at the given bit width
- Server tests in `tests/test_server.py` use `DummyModelProvider` mock + `http.server.HTTPServer`.

## Package

- Installed editable: `pip install -e .`.
- Hard dependencies: `mlx>=0.30.4` (Darwin only), `numpy`, `transformers>=5.0.0`, `sentencepiece`, `protobuf`, `pyyaml`, `jinja2`.
- Python 3.8+ required.
